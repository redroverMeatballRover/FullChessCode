...
public class Rook extends ChessPiece {	
	...
	public void deletePiece(ChessPiece[][] chess, int column1st, int row1st, int column2nd, int row2nd)
	{
	...
	//CHECK IF PATH IS CLEAR
		...
		public boolean capture(ChessPiece[][] pieces, int startY, int startX, int endY, int endX)
		{
			...
			if (pieces[endX][endY] != null) {
				...
				if (pieces[endX][endY].getColor().equals(pieces[startX][startY].getColor()))
				{
					...
					capture = checkAllSquaresBetweenInitialAndFinal(pieces, startY, startX, endY, endX, capture);	
		...
		private boolean rookMoveRight(ChessPiece[][] pieces, int startY, int endY, int endX, boolean pathIsClear, int distanceToCrossY) {
			...
			if (distanceToCrossY > 0 )
			{
				...
				for (int i = startY - 1; i > endY ; i--) //pathIsClear && i > endY ; i--)
				{
					//System.out.println("Space " + i + " at y:" + endY + " , x: " + i +  ": " +  pieces[endX][i] + " distanceToCross: " + (distanceToCrossY - i));
					//System.out.println(pieces[endX][i].getName());
					System.out.println("Square " + endX + ", " + i);
					if (pieces[endX][i] != null)
					{
