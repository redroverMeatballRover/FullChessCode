...
//		//checks end location for pieces. if piece is present, no matter what color, it will not capture it.
//		if (pieces[endX][endY] != null) 
//			pathIsClear = checkingFinalLocation(pieces, endY, endX);
//		else 
//			pathIsClear = checkSquaresBetweenInitialAndFinalLocations(pieces, startY, startX, endY, endX, pathIsClear);
//		return pathIsClear;
//	}

	//checks final location for any piece
	private boolean checkingFinalLocation(ChessPiece[][] pieces, int endY, int endX) 
	{
		boolean pathIsClear;
		System.out.println("BLOCKED - blocked by " + pieces[endX][endY].toString());
		pathIsClear = false;
		
		//if piece is matching color, BLOCKED. 
		//if not, check the rest of the path(below statments), and if still safe, capture
		return pathIsClear;
	}
